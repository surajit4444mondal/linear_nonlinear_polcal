...
def determine_antenna_leak_matrix(num_ant):#,low_amp_leak,high_amp_leak,mean_phase_leak, sigma_phase_leak):
    ...
    leak_amp_XY=data[:,0]
    leak_phase_XY=data[:,1]
    
    leak_amp_YX=data[:,2]
    leak_phase_YX=data[:,3]
    
    ant_leaks_XY=leak_amp_XY*np.exp(1j*leak_phase_XY)
    ant_leaks_YX=leak_amp_YX*np.exp(1j*leak_phase_YX)
    ...
    for i in range(num_ant):
        ant_leak_matrix[i,:,:]=np.array([[1,ant_leaks_XY[i]],[ant_leaks_YX[i],1]],dtype=complex)
...
msnames=['quartical/unpolarized_source_I10.ms',\
        'quartical/polarized_source_I50_Q1_U5_V0.ms',\
        'quartical/polarized_source_I50_Q5_U1_V0.ms',\
        'quartical/polarized_source_I50_Q4_U15_V0.ms',\
        'quartical/polarized_source_I50_Q15_neg_U5_V0.ms']

source_stokes=[[10,0,0,0],\
                [50,1,5,0],\
                [50,5,1,0],\
                [50,4,15,0],\
                [50,15,-5,0]]

for msname,stokes in zip(msnames,source_stokes):
    vis=get_visibilities(num_ant=30,source_stokes=stokes)#,low_amp_leak=0.01,\
